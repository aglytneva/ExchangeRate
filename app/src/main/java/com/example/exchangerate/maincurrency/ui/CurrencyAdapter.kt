...
class CurrencyAdapter(private val onAddToFavoriteCurrencyClicked: (Int) -> Unit) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(currencyData[position].favoriteCurrency == false) {
            holder.ivFavorite.setImageResource(R.drawable.ic_baseline_star_border_24)
        }
        else if (currencyData[position].favoriteCurrency == true) {
        ...
        holder.ivFavorite.setOnClickListener {
            onAddToFavoriteCurrencyClicked.invoke(position)
